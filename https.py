...
class SimpleHTTPServer:
    ...
    def start(
        ...
        if all_resources_downloaded:
            ...
            if use_https:
                key_path, certificate_path = self.load_certificates(
                ...
                context_factory = ssl.DefaultOpenSSLContextFactory(
                    str(key_path), str(certificate_path)
                )
            ...
            print(
                f"Server running on {self.host}:{self.port} ({'https' if use_https else 'http'})"
            )
            reactor.run()
        else:
...
def main():
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=443, help="Port to bind the server to."
    )
    parser.add_argument(
        "--url", type=str, required=True, help="URL to download and serve HTML from."
    )
    parser.add_argument(
        "--ssl_country", type=str, default="US", help="SSL certificate country."
    )
    parser.add_argument(
        "--ssl_state", type=str, default="CA", help="SSL certificate state."
    )
    parser.add_argument(
        "--ssl_locality",
        type=str,
        default="San Francisco",
        help="SSL certificate locality.",
    )
    parser.add_argument(
        "--ssl_org",
        type=str,
        default="MyOrganization",
        help="SSL certificate organization.",
    )
    parser.add_argument(
        "--domain_name",
        type=str,
        default="localhost",
        help="SSL certificate domain name.",
    )
    ...
    http_server.start(
        use_https=True,
        ssl_country=args.ssl_country,
        ssl_state=args.ssl_state,
        ssl_locality=args.ssl_locality,
        ssl_org=args.ssl_org,
